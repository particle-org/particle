...
public class NBTTagList extends NBTBase {
    ...
    public NBTTagList(byte tagType) {
    ...
    void read(DataInput input, int depth, NBTSizeTracker sizeTracker) throws IOException {
        ...
        if (depth > 512) {
            ...
            if (this.tagType == 0 && i > 0) {
                ...
                for (int j = 0; j < i; ++j) {
                    NBTBase nbtbase = createNewByType(this.tagType);
    ...
    public void appendTag(NBTBase nbt) {
        if (nbt.getId() == 0) {
            logger.info("Invalid TagEnd added to ListTag");
        } else {
            if (this.tagType == 0) {
                this.tagType = nbt.getId();
            } else if (this.tagType != nbt.getId()) {
                logger.info("Adding mismatching tag types to tag list");
                return;
            }
    ...
    public void set(int idx, NBTBase nbt) {
        if (nbt.getId() == 0) {
            ...
            logger.info("index out of bounds to set tag in tag list");
        }
    }
    ...
    public NBTTagString getStringValAt(int i) {
        if (i >= 0 && i < this.tagList.size()) {
            NBTBase nbtbase = (NBTBase) this.tagList.get(i);
            return nbtbase.getId() == 8 ? (NBTTagString) nbtbase : new NBTTagString();
        } else {
