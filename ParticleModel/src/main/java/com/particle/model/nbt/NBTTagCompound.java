...
public class NBTTagCompound extends NBTBase {
    ...
    void read(DataInput input, int depth, NBTSizeTracker sizeTracker) throws IOException {
        ...
        if (depth > 512) {
            ...
            while ((id = readType(input, sizeTracker)) != 0) {
                String s = readKey(input, sizeTracker);
                sizeTracker.read((long) (224 + 16 * s.length()));
                NBTBase nbtbase = readNBT(id, s, input, depth + 1, sizeTracker);
    ...
    public byte[] getByteArray(String key) {
        try {
            ...
            throw new RuntimeException(this.createCrashReport(key, 7, classcastexception));
        }
    }
    ...
    public int[] getIntArray(String key) {
        try {
            ...
            throw new RuntimeException(this.createCrashReport(key, 11, classcastexception));
        }
    }
    ...
    public NBTTagCompound getCompoundTag(String key) {
        try {
            ...
            throw new RuntimeException(this.createCrashReport(key, 10, classcastexception));
        }
    }
    ...
    public NBTTagList getTagList(String key, int type) {
        try {
            ...
            throw new RuntimeException(this.createCrashReport(key, 9, classcastexception));
        }
    }
    ...
    private String createCrashReport(final String key, final int expectedType, ClassCastException ex) {
        return key + " " + expectedType + " " + ex.toString();
    }
