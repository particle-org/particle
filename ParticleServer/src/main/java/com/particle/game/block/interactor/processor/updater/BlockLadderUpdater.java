...
import com.particle.model.math.BlockLadderFace;
...
@Singleton
public class BlockLadderUpdater implements IBlockWorldUpdater {
    ...
    public boolean onBlockUpdated(Level level, Player player, Block targetBlock, Vector3 targetPosition, BlockOperationType blockOperationType) {
        BlockLadderFace blockLadderFace = BlockLadderFace.getBlockLadderFace(targetBlock.getMeta());

        if (blockLadderFace != null) {
            switch (blockLadderFace) {
                case EAST:
