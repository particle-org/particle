...
@Singleton
public class FireworkRocketEntityProcessor implements IHitEntityProcessor {
    ...
    private class ColliderBlockCallback implements Consumer<Vector3f> {
        ...
        public void accept(Vector3f position) {
            explosionService.explosionAt(projectile.getLevel(), projectile, position, 7);
    ...
    private class ColliderEntityCallback implements Consumer<Entity> {
        ...
        public void accept(Entity target) {
            ...
            // 爆炸
            explosionService.explosionAt(projectile.getLevel(), damager, positionService.getPosition(target), 7);
