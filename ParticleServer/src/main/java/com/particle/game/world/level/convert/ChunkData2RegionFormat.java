...
@Singleton
public class ChunkData2RegionFormat implements ChunkData2RegionFormatApi {
    ...
    public NBTTagCompound toNbt(ChunkData chunkData) {
        ...
        for (ItemEntity itemEntity : itemEntitiesList) {
            ...
            TransformModule transformModule = TRANSFORM_MODULE_HANDLER.getModule(itemEntity);
            ...
            Vector3f position = transformModule.getPosition();
            nbtTagList.appendTag(new NBTTagDouble(position.getX()));
            nbtTagList.appendTag(new NBTTagDouble(position.getY()));
            nbtTagList.appendTag(new NBTTagDouble(position.getZ()));
            entityTag.setTag("Pos", nbtTagList);
