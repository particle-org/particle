...
public class CustomFileChunkProvider implements LevelProvider, LevelProviderMapper {

    private static Logger logger = LoggerFactory.getLogger(CustomFileChunkProvider.class);

    private CustomFileChunkReader customFileChunkReader;

    private ChunkData2NBTTagCompound chunkData2NBTTagCompound;

    public CustomFileChunkProvider(ChunkData2NBTTagCompound chunkData2NBTTagCompound, String location, String levelName) {
        this.customFileChunkReader = new CustomFileChunkReader(location, levelName);
    ...
    public ChunkData loadChunk(int x, int z) {
        DataInputStream chunkInputStream = customFileChunkReader.getChunkInputStream(x, z);
    ...
    public void saveChunk(ChunkData chunkData, boolean release) {
        DataOutputStream dataOutputStream = customFileChunkReader.getChunkOutputStream(chunkData.getxPos(), chunkData.getzPos());
