...
@Singleton
public class ItemEggPreUseProcessor implements IItemUseProcessor {
    ...
    public void process(Player player, ItemUseInventoryData itemUseInventoryData, InventoryActionData[] inventoryActionData) {
        ...
        ItemStack egg = this.playerInventoryAPI.getItem(playerInventory, playerInventory.getItemInHandle());

        // 校验物品
        if (egg == null || egg.getItemType() != ItemPrototype.EGG) {
            return;
        }
        ...
        if (this.playerService.getGameMode(player) == GameMode.SURVIVE) {
            ...
            if (egg.getCount() > 0) {
                this.playerInventoryAPI.setItem(playerInventory, playerInventory.getItemInHandle(), egg);
            } else {
        ...
        //射出抛射物
        this.entityRemoteAttackService.projectileShoot(player, egg, this.positionService.getDirection(player).getDirectionVector().multiply(30).add(0, 1, 0));
    }
}
