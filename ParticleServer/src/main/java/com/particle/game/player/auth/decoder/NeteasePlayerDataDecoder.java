...
public class NeteasePlayerDataDecoder {
    ...
    public static void decode(ClientChainData chainData, String playerByteData, int protocolVersion) {
        //读取额外信息
        JsonObject playerData = decodeToken(playerByteData);
        if (playerData != null) {
            ...
            if (protocolVersion >= AbstractPacketHandler.VERSION_1_13) {
                ...
                // 1.16
                if (playerData.has("SkinColor")) {
                    playerSkinData.setSkinColor(playerData.get("SkinColor").getAsString());
                }
                if (playerData.has("ArmSize")) {
                    playerSkinData.setArmSize(playerData.get("ArmSize").getAsString());
                }
                if (playerData.has("PersonaPieces")) {
                    for (JsonElement jsonElement : playerData.get("PersonaPieces").getAsJsonArray()) {
                        JsonObject jsonObject = jsonElement.getAsJsonObject();
                        String pieceId = jsonObject.get("PieceId").getAsString();
                        String pieceType = jsonObject.get("PieceType").getAsString();
                        String packId = jsonObject.get("PackId").getAsString();
                        boolean isDefault = jsonObject.get("IsDefault").getAsBoolean();
                        String productId = jsonObject.get("ProductId").getAsString();
                        playerSkinData.getPersonaPieceList().add(new PersonaPiece(pieceId, pieceType, packId, isDefault, productId));
                    }
                }
                if (playerData.has("PieceTintColors")) {
                    for (JsonElement jsonElement : playerData.get("PieceTintColors").getAsJsonArray()) {
                        JsonObject jsonObject = jsonElement.getAsJsonObject();
                        String pieceType = jsonObject.get("PieceType").getAsString();
                        List<String> colorList = new ArrayList<>();
                        for (JsonElement element : jsonObject.get("Colors").getAsJsonArray()) {
                            colorList.add(element.getAsString()); // remove #
                        }
                        playerSkinData.getPersonaPieceTintList().add(new PersonaPieceTint(pieceType, colorList));
                    }
                }
