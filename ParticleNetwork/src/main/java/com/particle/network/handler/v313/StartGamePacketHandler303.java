...
public class StartGamePacketHandler303 extends AbstractPacketHandler<StartGamePacket> {

    /**
     * 存储runTimeIdTable
     */
    public static PacketBuffer runtimeIdTableBufV303;

    private static List<String> exclude1_8 = new ArrayList<>();

    /**
     * 初始化block的runtimeId表
     */
    static {
        exclude1_8.add("minecraft:glow_stick");
        ...
        runtimeIdTableBufV303 = new PacketBuffer(tableLen + 10);
        runtimeIdTableBufV303.writeUnsignedVarInt(tableLen);
        for (BlockPrototype blockPrototype : blockPrototypes) {
            int maxMeta = blockPrototype.getMaxMetadata();
            if (exclude1_8.contains(blockPrototype.getName())) {
                // 非1.8的去除
                for (short i = 0; i < maxMeta; i++) {
                    runtimeIdTableBufV303.writeString("minecraft:element_1");
                    runtimeIdTableBufV303.writeLShort(i);
                }
            } else {
                for (short i = 0; i < maxMeta; i++) {
                    runtimeIdTableBufV303.writeString(blockPrototype.getName());
                    runtimeIdTableBufV303.writeLShort(i);
                }
            }
        }
    ...
    protected void doEncode(StartGamePacket dataPacket, int version) {
        ...
        // blockTable
        dataPacket.writeBytes(StartGamePacketHandler303.runtimeIdTableBufV303.getBuffer());
