...
public class StartGamePacketHandler extends AbstractPacketHandler<StartGamePacket> {

    /**
     * 存储runTimeIdTable
     */
    public static PacketBuffer runtimeIdTableBuf;
    ...
    public StartGamePacketHandler() {
        ...
        runtimeIdTableBuf = new PacketBuffer(tableLen + 10);
        runtimeIdTableBuf.writeUnsignedVarInt(tableLen);
        for (BlockPrototype blockPrototype : blockPrototypes) {
            int maxMeta = blockPrototype.getMaxMetadata();
            for (short i = 0; i < maxMeta; i++) {
                runtimeIdTableBuf.writeString(blockPrototype.getName());
                runtimeIdTableBuf.writeLShort(i);
            }
        }
