...
import com.particle.network.encoder.BlockListEncoderV410;
import com.particle.network.encoder.GameRulesHandler;
import com.particle.network.encoder.ItemListEncoderV410;
...
public class StartGamePacketHandler410 extends AbstractPacketHandler<StartGamePacket> {

    private GameRulesHandler gameRulesHandler = GameRulesHandler.getInstance();

    private BlockListEncoderV410 blockListEncoder = new BlockListEncoderV410(BlockNBTJsonReader.read("block_state_list_V1_16.json"));
    private ItemListEncoderV410 itemListEncoder = new ItemListEncoderV410(ItemInfoReader.read("avaliable_item_ids.json"));
    ...
    private LevelSettings decodeLevelSettings(DataPacket dataPacket, int version) {
        ...
        // 1.16
        levelSettings.setSpawnBiomeType(dataPacket.readLShort());
        levelSettings.setUserDefinedBiomeName(dataPacket.readString());
    ...
    protected void doEncode(StartGamePacket dataPacket, int version) {
        ...
        // 1.16
        dataPacket.writeBoolean(dataPacket.isInventoryServerAuthoritative());

        // only for netease-version
        if (isChsVersion) {
            dataPacket.writeSignedVarInt(dataPacket.getRewindHistorySize());
        }
    }
    ...
    private void doEncodeLevelSettings(DataPacket dataPacket, int version, LevelSettings levelSettings) {
        ...
        // 1.16
        dataPacket.writeLShort(levelSettings.getSpawnBiomeType());
        dataPacket.writeString(levelSettings.getUserDefinedBiomeName());
