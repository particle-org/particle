...
public class ItemStackEncoder extends ModelHandler<ItemStack> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ItemStackEncoder.class);

    /**
     * 单例对象
     */
    private static final ItemStackEncoder INSTANCE = new ItemStackEncoder();

    /**
     * 获取单例
     */
    public static ItemStackEncoder getInstance() {
        return ItemStackEncoder.INSTANCE;
    }
    ...
    public void encode(DataPacket dataPacket, ItemStack itemStack, int version) {
        ...
        if (nbtTagCompound == null) {
            ...
            try {
                ...
                if (nbt != null) {
                    dataPacket.writeLShort((short) nbt.length);
